  ...
  <div class="container" style="margin-bottom: 100px">
    ...
    <b-row>
     ...
     <b-button class="mt-3" variant="primary" block @click="selectResult2"
      ...
      >
        <b-button class="mt-3" variant="primary" block @click="movePrimaryNodestoSide">movePrimaryNodestoSide </b-button>
      <b-button class="mt-3" variant="primary" block @click="consoleStuff"
        >consoleStuff
      </b-button>      
      <!-- <br> -->
      <b-button class="mt-3" variant="primary" block @click="getNodeParentGrandparentData"
        >getNodeParentGrandparentData 
      </b-button>
      <b-button class="mt-3" variant="primary" block @click="collapseChildNodes"
        >collapseChildNodes 
      </b-button>
      <!-- <br>  -->
      ...
      <b-button class="mt-3" variant="primary" block @click="coseLayout">coseLayout </b-button>
        <b-button class="mt-3" variant="primary" block @click="breadthfirstLayout">breadthfirstLayout </b-button>
        <br />        
        <b-button class="mt-3" variant="secondary" block @click="sizeByDegree">sizeByDegree </b-button>    
        <b-button class="mt-3" variant="secondary" block @click="organizeNodes">organizeNodes </b-button>    
  ...
  methods: {
    ...
    genEdges(filterCytoData) {
      ...
      for (let i = 0; i < filterCytoData.length; i++) {
        ...
        if (edgesCheck.indexOf(edgeCheck) == -1 && primaryEdge1 != edgeCheck && primaryEdge2 != edgeCheck) {
          let el = {};
          el.data = {};
    ...
    collapseChildNodes(){
      ...
      for (let index = 0; index < nodeData.length; index++) {
        ...
        let location = this.cyInstance.$(parentNodeSelectName).renderedPosition()
    ...
    makeParents (){
      ...
      for (let i = 0; i < nodesWithParent.length; i++) {
        ...
        if(parentNodeCheck.indexOf(nodeParentID) == -1){
          ...
          let parentGroupNodes = nodeData.filter(x => x.data.parentGroup == nodeParentID)
          ...
          if(parentGroupNodes.length > 1){
            ...
            parentNode.data.elbowChildIDs = parentGroupNodes.map(x => x.data.id)
            console.log("parentNode.data.elbowChildIDs")
            console.log(parentNode.data.elbowChildIDs)
            // GET ELBOW NEIGHBORS TO MAKE EDGES
            let elbowNeighborIDs = this.cyInstance.$("#" + node.data.id).neighborhood().nodes().jsons().map(x => x.data.id)   
            console.log("elbowNeighborIDs")         
            console.log(elbowNeighborIDs)  
            console.log(this.cyInstance.$("#" + node.data.id).neighborhood().jsons())       
            ...
            for (let n = 0; n < parentGroupNodes.length; n++) {
              ...
              let elbowRenderedPosition = this.cyInstance.$(getNodeID).position()
    ...
    sizeByDegree(){
      ...
      for (let i = 0; i < nodeData.length; i++) {
        ...
        let animateChanges = 
        {
          style: { 'height': adjustHeight, 'width': adjustWidth, visibility: 'hidden' }, 
          // style: { 'height': adjustHeight, 'width': adjustWidth, opacity: 0 }, 
          duration: 1000
        }
        this.cyInstance.nodes(nodeGetID).animate(animateChanges)
        console.log("change height = ", this.cyInstance.$(nodeGetID).style().height)
    ...
    organizeNodes(){
      let nodeData = this.cyInstance.nodes().jsons()
      console.log("nodeData = ", nodeData)
      let parentData = nodeData.map(x => x.data.parent)
